...
class Boards(Routes):
    ...
    # --- NEW TASK --- START
    _newTaskBtnXpath = "//tr[1]/th[1]/span[1]"
    ...
    def edit_color(self, colorOrder):
        self.validate.get_web_element(self._boardAppColorOptionXpath.format(colorOrder)).click()
    ...
    def validate_boardName_Edit(self, name):
        if self.validate.is_element_found_by_xpath(self._BoardNameXpath.format(name)):
            print("Board name is {0}".format(name))
    ...
    # def open_TaskinWeek(self, name):
    #     self.validate.get_web_element(self._TaskinweekCalendar)
    ...
    # def ViewTaskInCalendar(self,name):
    #     self.validate.get_web_element(self.)
